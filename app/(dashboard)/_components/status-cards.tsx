...
export const StatusCards = () => {
    ...
    const mutateSensorStatus = useMutation(api.sensor.updateSensorStatus)
    const startSystem = () => {
        for (let i = 0; i < 8; i++) {
            mutateSensorStatus({ sensorId: i, newStatusCode: SensorStatus.starting });
        }
    };
    const shutdownSystem = () => {
        for (let i = 1; i < 8; i++) {
            mutateSensorStatus({ sensorId: i, newStatusCode: SensorStatus.shuttingDown });
        }
    };
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
            ...
            >
                <CardHeader className="pb-3">
                    ...
                    <CardDescription className="max-w-lg text-balanced leading-relaxed">
                        {status_text}
                    </CardDescription>
                </CardHeader>
                {/* <CardContent className="mt-2">
                    <Button
                        onClick={num_finished === 8 ? shutdownSystem : startSystem}
            ...
            <Card x-chunk="dashboard-05-chunk-1">
                <CardHeader className="pb-2">
                    <CardDescription>Current step progress</CardDescription>
                    <CardTitle className="text-xl">{current_step_progress}%</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="text-xs text-muted-foreground">
                        {current_step?.map(({ description }) => description)[0] || "Nothing to run"}
                    </div>
                </CardContent>
                <CardFooter>
                    <Progress value={current_step_progress} aria-label="current progress" />
                </CardFooter>
            </Card>
