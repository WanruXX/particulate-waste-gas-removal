...
export const SensorNode = ({
    ...
    const sensor_data = useQuery(api.sensor.get, { sensorId: sensor_id });
    ...
    if (status == SensorStatus.starting || status == SensorStatus.shuttingDown) {
    ...
    if (status == SensorStatus.starting || status == SensorStatus.shuttingDown) {
        icon_color = "#35a6db";
    } else if (status == SensorStatus.on) {
        icon_color = "#3fd46e";
        const outputs = useQuery(api.sensor.getAsInput, { sensorId: selected_sensor })?.map(({ t, pressure, max_t, min_p }) => [t, pressure, max_t, min_p]) || [];
